...
struct bt_context *bt_ctxt; 
bool ble_connected = false;
bool connectionless = false;
bool connection_base = false;
bool ext_adv_actiavte = false;
bool for_echo_server = false;
...
struct net_if *bt_iface;
struct net_buf *buf_clone;
u16_t pkt_lenght;
u16_t data_size = 0;
static enum net_verdict net_bt_recv(struct net_if *iface, struct net_pkt *pkt)
{
//	printk("iface %p pkt %p len %zu", iface, pkt, net_pkt_get_len(pkt));
//	printk("net_bt_recv \n");
...
static enum net_verdict net_bt_send(struct net_if *iface, struct net_pkt *pkt)
{
	...
	if(connectionless){
		...
		if(ext_adv_actiavte == false){  // activate adv only
			...
			struct bt_data ad[] = {
				BT_DATA(BT_DATA_MANUFACTURER_DATA, packet, 20),
			};
...
//we initialize the L2 layer here
NET_L2_INIT(BLUETOOTH_L2, net_bt_recv, net_bt_send, net_bt_reserve,
	    net_bt_enable);

/************************************************************************************************/		

/*********** initiated by bt_context_data intiated by bt_context_data in NET_DEVICE_INIT ***********/
...
//	bt_ctxt = CHAN_CTXT(chan);
	...
	if(!connectionless){
		/* Set iface down */
//		net_if_carrier_down(ctxt->iface);  	// can be used to switch from connection to connectioinless
	}
...
static struct net_buf *ipsp_alloc_buf(struct bt_l2cap_chan *chan)
{
	...
	// allocate buffer for receiveing packet 
...
/********* functions intiated by bt_if_api used in NET_DEVICE_INIT ***************/
...
/***** callback functions which are configured during the initialization of the interface by net_bt_init
     during the registration of the server  *********/
...
/******** helping function of bt_shell to advertise, scan and connect **********************/
...
static void device_found(const bt_addr_le_t *addr, s8_t rssi, u8_t type,
			 struct net_buf_simple *ad, struct net_buf *buf)
{
	...
	//	printk("BT_LE_ADV_IND \n");
		...
		printk("aux packet \n");
		struct net_pkt *pkt;
		struct net_buf *buf1 = buf->frags;

//		buf->data = ad->data;
//		buf->len = 128;
//		memset(packet, 0, 1299);
	//	memcpy(packet, buf1->data , buf1->len);
	//	buf->data = old_packet;
...
//			net_buf_frags_len(buf));
	...
	/* steps for 1280  bytes data packets */
...
{
	int err;
	// device found function is used as a callback. this function will be called on receiveng new packet.
	// can be used on scanner side and parse the ext_edv packet and given to 
	// static enum net_verdict net_bt_recv(struct net_if *iface, struct net_pkt *pkt)
...
static int bt_switch(u32_t mgmt_request, struct net_if *iface,
			 void *data, size_t len)
{
	struct bt_context *ctxt = net_if_get_device(iface)->driver_data;
...
static void connected(struct bt_conn *conn, u8_t err)
{
	...
	// L2CAP connection is established here 
...
static void disconnected(struct bt_conn *conn, u8_t reason)
{
	...
	NET_DBG("not connected: %s (reason %u)\n", addr, reason);
#endif
...
//			bt_active_scan();
		...
		/* Set iface down */
//		net_if_carrier_down(bt_ctxt->iface);  	// can be used to switch from connection to connectioinless
	}
//#endif
...
NET_MGMT_REGISTER_REQUEST_HANDLER(NET_REQUEST_BT_SWITCH, bt_switch);
#endif


/*
NET_DEVICE_INIT(dev_name, drv_name, init_fn, data, cfg_info, prio, api, l2, l2_ctx_type, mtu)
