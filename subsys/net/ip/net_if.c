...
send:
	...
	/* these are the changes needs to be done for connetion less but needs to think about stopping an advertising */
//	if(ble_connected)
	{
...
/*		if (net_pkt_family(pkt) == AF_INET6) {
			...
			if (!pkt) {
				...
				if(((*(frag->data)) == 0x7E) || ((*(frag->data)) == 0x60)||((*(frag->data)) == 0x06)){
					...
					struct bt_data ad[] = {
						BT_DATA(0xff, &packet[0], frag->len),
					};
			...
			//		net_if_queue_tx(context->iface, pkt);
...
/*		if (!net_6lo_compress(pkt, true, NULL)) {
		...
		struct bt_data ad[] = {
			BT_DATA(0xff, &packet[0], len),
		};

		err= bt_le_ext_adv_start(BT_LE_ADV_NCONN, ad, ARRAY_SIZE(ad),&packet, len,
					sd, ARRAY_SIZE(sd));
