...
static u16_t pointer_index = 0;		// for chain paket
struct aux_pkt chain_pkt;
...
static int set_ext_advertise_enable(bool enable)
{
	struct net_buf *buf;
	int err;
	...
	if (enable) {
		...
		//net_buf_add_u8(buf, 0x01);						// number of advertising set enable
...
struct net_buf_simple aux_pkt;
u8_t copied_data[1500];
u8_t num_reports;
u16_t buffer_size1 = 0;
u8_t temp_data[270];
static void le_adv_ext_report(struct net_buf *buf)
{
	...
	struct net_buf_simple *buf_sample;
	...
	while (num_reports--) {
		...
		buf_len = buf->len;
		buf->len = info->length;
/*		memset(&copied_data[0], 0, 1500);
		memcpy(&copied_data[0], &buf->b.data, info->length);
		memcpy(&buf->b.data, &copied_data[0], info->length);
		aux_pkt.data = &copied_data[0];
		aux_pkt.len = info->length;
*/		
...
/*
		if (scan_dev_found_cb) {
			...
			//	buf->len = info->length;   // remove 0x81 0xFF 

			//	net_buf_simple_save(&buf->b, &state);

				// steps for 1280  bytes data packets 
			//	net_buf_add_mem(buf,mfg_data, sizeof(mfg_data));
			//	net_buf_pull(buf, info->length);
...
int ext_set_data(u16_t data_lenght,u8_t *data){
	...
	while(i <= fragment_count){
		...
		if(data_lenght > 246){
			...
			packet_size = data_lenght;
		}
		
		if((i == 0)&& (fragment_count == 0)){
			operation = 3;  //complete data
		}else if(i == 0){
			operation = 1;	//1st fragment
		}else if(i == fragment_count){
		    ...
		    const struct bt_data *sd, size_t sd_len)
{
	...
	if (param->options & BT_LE_ADV_OPT_CONNECTABLE) {
		...
		/* if want legacy adv then just add  BT_HCI_LE_ADV_PROP_LEGACY in set_param.props */
		if (sd) {														
			...
			set_param.props = BT_HCI_LE_ADV_PROP_TX_POWER ; //| BT_HCI_LE_ADV_PROP_DIRECT; 						
		}
...
*/
	...
	err = set_ext_advertise_enable(true);
	if (err) {
		return err;
	}
...
int bt_le_ext_adv_stop(void)
{
	...
	err = set_ext_advertise_enable(false);
	if (err) {
		return err;
	}
