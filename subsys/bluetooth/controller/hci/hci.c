...
static void read_supported_commands(struct net_buf *buf, struct net_buf **evt)
{
	...
	/* LE Set Ext Adv Params, LE Set Ext Adv Data, LE Set Ext Adv Enable,Read Num of Supported Adv Sets,Read Max Adv Data Length */
...
static void le_set_ext_adv_param(struct net_buf *buf, struct net_buf **evt)
{
	...
	struct bt_hci_rp_le_set_ext_adv_param *ccst;
	u16_t min_interval;
	u8_t status;

	min_interval = sys_le16_to_cpu(cmd->prim_min_interval);

	status = ll_ext_adv_params_set(cmd->handle, cmd->props, min_interval,
				   cmd->own_addr_type, cmd->direct_addr.type,
				   &cmd->direct_addr.a.val[0], cmd->prim_channel_map,
				   cmd->filter_policy, 0, cmd->prim_adv_phy, cmd->sec_adv_max_skip,  cmd->sec_adv_phy,cmd->sid, 0);
	...
	if ((adv->type == PDU_ADV_TYPE_EXT_IND) && adv->len) {
		...
		if(h->adi){
			...
			if(adi_ptr->sid == 0x04){
				...
				adi_present = true;					// packet on primary channel
			}
			ptr += sizeof(*adi_ptr);
		}
...
/*			adv_info->length = data_offset-2;
			memcpy(&adv_info->data[0], &copied_data, data_offset);
		//	prssi = &adv_info->data[0] + data_len;
		//	*prssi = 00;
*/
		}else{
