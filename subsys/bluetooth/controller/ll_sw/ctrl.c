...
u32_t start_time_5;
u32_t stop_time_37;
u32_t start_time_ticker;
u32_t stop_time_ticker;
u64_t Time_difference_ticker;
u32_t stop_time_ticker1;
u32_t Time_difference_ticker1;

u32_t start_time_int;
u32_t stop_time_int;
u32_t Time_difference_int;
u32_t event_time_int;

//u32_t stop_time_38;
u32_t stop_time_39;
u32_t stop_time_aux;
u32_t stop_time_aux1;
u32_t cycles_spent_37;
//u32_t cycles_spent_38;
//u32_t cycles_spent_39;
//u32_t cycles_spent_aux;
//u32_t cycles_spent_aux1;
u32_t us_spent_37;
u32_t us_spent_38;
u32_t us_spent_39;
u32_t us_spent_aux;
...
u32_t us_spent_3;
u32_t us_spent_4;
u32_t Time_difference;
u32_t us_spent_aux1;
static u16_t pointer_offset = 0; 		// for aux packet buffer p

bool radio_ext_adv_enable_flag = false;


u64_t start_dynamic_cal;
u64_t stop_dynamic_cal;
u64_t final_dynamic_cal = 0;
u32_t dynamic_offset = 0;
u64_t last_dynamic_cal = 0;

static u8_t AUX_PACKET = 0;
K_TIMER_DEFINE(my_status_timer, NULL, NULL);
static u8_t pckt_send = 0;
static u8_t aux = 0;
static u16_t aux_offset = 0;
static u8_t chan_index = 0;
static u8_t chan_case = 0;
static u8_t aux_scan = 0;
static u8_t aux_sid = 0;
static u8_t reset = 0;
static u8_t counter = 0;

static u8_t chain_flag = 0;
...
struct ext_advertiser{
	...
	u8_t  aux_pckt_chan;
	u32_t ticker_offset;
	u32_t last_dynamic_cal;
	u8_t aux_chain_count;
	u8_t aux_chain_count_current;
};

struct ext_scanner{
	u8_t chan_index;
	u64_t chan_off;
	u64_t chan_off_debug;
	u64_t chan_off_calculation;
	u32_t chan_off_pckt;
	u8_t off_unit;
	u8_t aux_pckt_flag;
	u8_t event_flag;
...
u32_t diff;
u32_t diffns, event_time_diff;
static struct k_work_q work_q;
struct ext_scanner aux_scanning;
...
struct advertiser {
	...
	struct radio_adv_data aux_adv_data;					//Aux packet
	struct radio_chain_adv_data chain_adv_data;					//Aux packet
...
static struct {
	...
	struct ext_advertiser ext_advertiser;
	struct ext_scanner ext_scanner;
...
struct ext_adv_aux_ptr scan_aux_ptr;
...
static void ext_adv_setup(void);/*************************************************************************************/
static void event_adv(u32_t ticks_at_expire, u32_t remainder, u16_t lazy,
		      void *context);
/***********************************************************************************/
static void event_ext_adv(u32_t ticks_at_expire, u32_t remainder, u16_t lazy,
		      void *context);
/*********************************************************************************************************/
...
static void event_stop(u32_t ticks_at_expire, u32_t remainder,
		       u16_t lazy, void *context);
static void event_ext_scan_prepare(u32_t ticks_at_expire, u32_t remainder,
			      u16_t lazy, void *context);
static void event_ext_scan(u32_t ticks_at_expire, u32_t remainder, u16_t lazy,
		      void *context);
...
//	role_disable(RADIO_TICKER_ID_EXT_ADV, RADIO_TICKER_ID_EXT_ADV_STOP);	/*******************************************************************************/
...
static void aux_packet_function(struct k_work *work) /*********************************************************************************************/
{

/*	stop_time_ticker1 = k_cycle_get_32();
	

		
	
	diff = stop_time_ticker1 - start_time_ticker;
	diffns = SYS_CLOCK_HW_CYCLES_TO_NS(diff);
	Time_difference_ticker1 = diffns/1000;

	u8_t ret; 
	ret = ticker_start(						//this ticker was for closing the window.instead we ll use it for Aux packet
		RADIO_TICKER_INSTANCE_ID_RADIO,
		RADIO_TICKER_USER_ID_WORKER,
	//	RADIO_TICKER_USER_ID_APP,
		RADIO_TICKER_ID_SCAN_STOP, _radio.ext_scanner.ticks_now,
		0,
		TICKER_NULL_PERIOD, TICKER_NULL_REMAINDER,
		TICKER_NULL_LAZY, 
		TICKER_NULL_SLOT,
	//	HAL_TICKER_US_TO_TICKS(100),
		event_ext_scan_prepare,
	//	event_ext_scan,
		NULL,
				...
				u8_t rssi_ready)
{
	...
	/* Ext adv packet extraction to get aux ptr data */
	if(pdu_adv_rx->type == 0x07){			//PDU_ADV_TYPE_EXT_IND
		...
		struct ext_adv_aux_ptr *ap;				// aux pt
		u8_t *ptr;
		u8_t ticker_status;
		
		u16_t did;
		u64_t offset_aux = 0x24;
		
		
	//	printk("entered in isr........");
		p = (void *)&pdu_adv_rx->adv_ext_ind;
...
*/		
		...
		//	aux_sid = sid;
			...
			if(_radio.ext_scanner.off_unit == 0 ){
				...
				offset_aux = HAL_TICKER_US_TO_TICKS(_radio.ext_scanner.chan_off);
			}else{
 		...
 		//	k_work_submit(&aux_scanning.work);
			// k_delayed_work_submit(&aux_scanning.work_d,(u32_t)20);
...
/*			ret = ticker_start(						//this ticker was for closing the window.instead we ll use it for Aux packet
				...
				//	RADIO_TICKER_USER_ID_APP,
					RADIO_TICKER_ID_SCAN_STOP, _radio.ext_scanner.ticks_now,
				//	0,
					HAL_TICKER_US_TO_TICKS(_radio.ext_scanner.chan_off- 1000),
				//	TICKER_NULL_PERIOD,
...
/*	ticker_start(RADIO_TICKER_INSTANCE_ID_RADIO,
				     ...
				     HAL_TICKER_US_TO_TICKS(											//max interval 1.28s for directed
					(1280 * 1000) +
...
/*	//			ret = ticker_start(
					RADIO_TICKER_INSTANCE_ID_RADIO,
					RADIO_TICKER_USER_ID_WORKER,
					RADIO_TICKER_ID_SCAN, ticker_ticks_now_get(),
					0,
					TICKER_NULL_PERIOD, TICKER_NULL_REMAINDER,
					TICKER_NULL_LAZY,  HAL_TICKER_US_TO_TICKS(1000),
					event_ext_scan_prepare, (void *)STATE_STOP,
					ticker_success_assert, (void *)__LINE__);
...
static inline u32_t isr_close_adv(void)
{
	...
	if(radio_ext_adv_enable_flag ==  true){
		...
		if((chain_flag ==1) &&
				(_radio.ext_advertiser.aux_chain_count_current != _radio.ext_advertiser.aux_chain_count)){
	...
	/*			ticker_status =
				...
				if((ticker_status == TICKER_STATUS_SUCCESS)){
					...
					LL_ASSERT((ticker_status1 == TICKER_STATUS_SUCCESS) ||
						(ticker_status1 == TICKER_STATUS_BUSY) ||
						(_radio.ticker_id_stop ==
						RADIO_TICKER_ID_ADV_STOP));
				}
...
static void chan_set(u32_t chan)
{
	...
	if(pdu->type == PDU_ADV_TYPE_EXT_IND){
		...
		static u8_t repeat =0;
		static u8_t flag = 0;
		switch (chan) {
		case 37:

			/* Time calculation for debugging */
		...
		case 38:
			...
			/* Time calculation for debugging */
		//	stop_time_38 = k_cycle_get_32();
		//	cycles_spent_38 = stop_time_38 - start_time;
		//	us_spent_38 = SYS_CLOCK_HW_CYCLES_TO_NS(cycles_spent_38)/1000;
			start_time = k_cycle_get_32();
...
/*							case 1:	
								...
								radio_freq_chan_set(10);
								break;
							case 2:	
								stop_time_aux1 = k_cycle_get_32();
								us_spent_1 = SYS_CLOCK_HW_CYCLES_TO_NS(stop_time_aux1 - start_time_5)/1000;
								start_time_5 = k_cycle_get_32();
								radio_freq_chan_set(10);
								break;
							case 3:	
								stop_time_aux1 = k_cycle_get_32();
								us_spent_2 = SYS_CLOCK_HW_CYCLES_TO_NS(stop_time_aux1 - start_time_5)/1000;
								start_time_5 = k_cycle_get_32();
								radio_freq_chan_set(10);
							//	start_time_5 = k_cycle_get_32();
								break;
							case 4:	
								stop_time_aux1 = k_cycle_get_32();
								us_spent_3 = SYS_CLOCK_HW_CYCLES_TO_NS(stop_time_aux1 - start_time_5)/1000;
								start_time_5 = k_cycle_get_32();
								radio_freq_chan_set(10);
								break;
							case 5:	
								stop_time_aux1 = k_cycle_get_32();
								us_spent_4 = SYS_CLOCK_HW_CYCLES_TO_NS(stop_time_aux1 - start_time_5)/1000;
								radio_freq_chan_set(10);
								break;
*/
		default:
...
void radio_event_ext_adv_prepare(u32_t ticks_at_expire, u32_t remainder,
			     u16_t lazy, void *context)
{
	ARG_UNUSED(lazy);
	ARG_UNUSED(context);
	...
	event_common_prepare(ticks_at_expire, remainder,
			     ...
			     &_radio.advertiser.hdr.ticks_active_to_start,
			     _radio.advertiser.hdr.ticks_preempt_to_start,
			     RADIO_TICKER_ID_ADV, event_ext_adv, NULL);
	...
	if(pdu->type == PDU_ADV_TYPE_EXT_IND){
		...
		if(pdu->type == PDU_ADV_TYPE_EXT_IND){
			...
			if(h->aux_ptr){
				...
				//	_radio.ext_advertiser.chan_offset  = offset/300;
				}else{
					ap->offs = offset /30;
...
//					[_radio.advertiser.aux_adv_data.last];
	...
	switch(_radio.ext_advertiser.aux_chain_count_current){
		case 0:
			...
			break;
		case 1:
			copy_pdu = (struct pdu_adv *)&radio_chain_adv_data->data[257];
			memcpy(aux_pdu, copy_pdu, 257);
			chan_set(1);
			break;
		case 2:
			copy_pdu = (struct pdu_adv *)&radio_chain_adv_data->data[514];
			memcpy(aux_pdu, copy_pdu, 257);
			chan_set(2);
			break;
		case 3:
			copy_pdu = (struct pdu_adv *)&radio_chain_adv_data->data[771];
			memcpy(aux_pdu, copy_pdu, 257);
			chan_set(3);
			break;
		case 4:
			copy_pdu = (struct pdu_adv *)&radio_chain_adv_data->data[1028];
			memcpy(aux_pdu, copy_pdu, 257);
			chan_set(4);
			break;
		case 5:
			copy_pdu = (struct pdu_adv *)&radio_chain_adv_data->data[1285];
			memcpy(aux_pdu, copy_pdu, 257);
			chan_set(5);
			break;
		case 6:
	...
	copy_pdu = (struct pdu_adv *)&radio_chain_adv_data->data[257];
//	aux_pdu = (void *)_radio.advertiser.chain_adv_data.data
//						[_radio.advertiser.chain_adv_data.last];

  /* aux chain packets need to be filled with aux_ptr field in order to transmit chain packets */
...
static void event_ext_adv(u32_t ticks_at_expire, u32_t remainder,
		      u16_t lazy, void *context)
{
	u32_t remainder_us;
		      ...
		      void *context)
{
	...
	if(aux_scan){
		...
		//	_radio.ext_scanner.chan_off_debug = _radio.ext_scanner.chan_off_calculation;
...
*/		//	chan_set(_radio.ext_scanner.chan_index);
		...
		//	printk("chann :3 \n");
			...
			_radio.ext_scanner.chan_off_calculation = _radio.ext_scanner.chan_off;
		//	_radio.ext_scanner.chan_off_debug  = 0;
		}
	}else{
...
/*	if(_radio.ext_scanner.event_flag){
		chan_set(3);
		aux_scan = 0;
	stop_time_ticker1 = k_cycle_get_32();
	...
	diffns = SYS_CLOCK_HW_CYCLES_TO_NS(diff);
	Time_difference_ticker1 = diffns/1000;
	//	_radio.ext_scanner.event_flag = 0;
...
/*
	stop_time_ticker = k_cycle_get_32();
	event_time_diff = (stop_time_ticker - start_time_ticker)/1000;
		
	Time_difference_ticker = SYS_CLOCK_HW_CYCLES_TO_NS(event_time_diff);
*/
	...
	event_common_prepare(ticks_at_expire, remainder,
			     ...
			     &_radio.scanner.hdr.ticks_active_to_start,
			     _radio.scanner.hdr.ticks_preempt_to_start,
			     RADIO_TICKER_ID_SCAN, event_ext_scan, NULL);
...
//	printk("Ticker stop \n");
	...
	chan_set(_radio.ext_scanner.chan_index);
	_radio.ext_scanner.event_flag++;
	if(_radio.ext_scanner.event_flag == 2){
		reset = 1;
		_radio.ext_scanner.event_flag = 0;
	//	k_work_submit(&aux_scanning.work);
	}
...
struct radio_adv_data *radio_aux_adv_data_get(void)
{
	return &_radio.advertiser.aux_adv_data;
}

struct radio_ext_adv_data *radio_chain_adv_data_get(void)
{
	return &_radio.advertiser.chain_adv_data;
}
...
u32_t radio_ext_adv_enable(u8_t phy_p, u16_t interval, u8_t chan_map,
		       u8_t filter_policy, u8_t rl_idx, u8_t max_skip, u8_t aux_chain_count)
{
	u32_t volatile ret_cb = TICKER_STATUS_BUSY;
	u32_t ticks_slot_offset;
	struct connection *conn;
	...
	if (_radio.advertiser.is_hdcd) {
		...
		// = 30 * max_skip; //to compensate the diff btween ext packet and aux packet. without this it shifts 30 *max_skip microsec
		...
		ret=	
			ticker_start(RADIO_TICKER_INSTANCE_ID_RADIO,
				    ...
				    HAL_TICKER_US_TO_TICKS(	ticks_slot_offset +			
							    376),
				     radio_event_ext_adv_prepare, NULL,
				     ticker_if_done, (void *)&ret_cb);
			...
			u8_t rpa_gen, u8_t rl_idx)
{
	...
	k_work_init(&aux_scanning.work, aux_packet_function); 		/***************************************************************************/
...
//	start_time_ticker = k_cycle_get_32();
	...
	ret = ticker_start(RADIO_TICKER_INSTANCE_ID_RADIO,
			RADIO_TICKER_USER_ID_WORKER, RADIO_TICKER_ID_SCAN,

			_radio.ext_scanner.ticks_now,0,
		//	HAL_TICKER_US_TO_TICKS(_radio.ext_scanner.chan_off), 
		//	 TICKER_NULL_PERIOD,
			HAL_TICKER_US_TO_TICKS((u64_t) _radio.scanner.interval * 625),
		//	TICKER_NULL_REMAINDER,
			HAL_TICKER_REMAINDER((u64_t) (_radio.scanner.interval * 625)),
			TICKER_NULL_LAZY,
			(_radio.scanner.slot_offset +
			_radio.scanner.hdr.ticks_slot),
